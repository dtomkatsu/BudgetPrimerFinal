...
The primer's authored text lives in content.md ("## key" sections), which is kept
...
def parse_content(path: Path) -> dict:
    ...
    if not matches:
        raise ContentError(f"{path.name}: no '## key' sections found")
...
def paragraph(block: str) -> str:
    ...
    return md_inline(" ".join(l.strip() for l in block.splitlines() if l.strip()))
...
class Content:
    ...
    def raw(self, key: str) -> str:
        if key not in self._raw:
            raise ContentError(
                ...
                f"  The Google Doc must keep every '## key' heading intact.")
        self._used.add(key)
    ...
    def text(self, key: str) -> str:
        ...
        return " ".join(l.strip() for l in self.raw(key).splitlines() if l.strip())

    def list(self, key: str) -> list[str]:
        return bullets(self.raw(key))

    def lines(self, key: str) -> list[str]:
        return [l.strip() for l in self.raw(key).splitlines() if l.strip()]
